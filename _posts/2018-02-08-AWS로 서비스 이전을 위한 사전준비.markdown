...
일반적으로 보안상 기본적으로 설정되어 있는 `/var/html` 을 사용하진 않으므로 경로를 바꿔준다.

1. 아파치 설정파일 `httpd.conf` 를 직접 바꾼다.
2. 별도의 설정파일을 아파치 설정파일에 include 하여 사용자 설정파일에 가상호스트로 경로를 바꿔준다.
...
<VirtualHost *:80>
    ...
    #ErrorLog logs/minda.com-error_log
    #CustomLog logs/minda.com-access_log combined
...
변경내용 확인
{% highlight bash %}
ls -alZ /home/u4kano/public_html/
//output
...
[6]: http://smc0210.github.io/assets/post/20180208-1-7.png
